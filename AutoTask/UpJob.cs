...
namespace AutoTask
{
    ...
    {
        public int ExecJob(JobPara jobPara, List<Datas> dsData)
        {
            #region 注释了
            ...
            //        {
            //            GetContext.WriteLog(LogType.Info, "京东供应商信息回传", "数据源中缺少serverUrl、accessToken、appKey、appSecret、Pin!");
            //            return -1;
            //        }
            //        else
            //        {
            ...
            //                if (string.IsNullOrEmpty(insertresult))
            //                {
            //                    allResult += $"\r\n仓库调拨入库单{dsData[i].DataMain.Rows[0]["LSH"].SqlDataBankToString()}成功";
            //                }
            //                else
            //                {
            //                    sqlHelper.ExecSql($"insert into ERRORDATA_XH_JDWMS(GUID,ERRORNO,ERRORTYPE,MAINFIELD,ERRORDATE,ERRORMSG) values('{Guid.NewGuid():N}',{dsData[i].DataMain.Rows[0]["LSH"].SqlDataBankToString()},'dbrkd','LSH',{createdate},'{returnValue.response.content.message}')");
            //                    allResult += $"\r\n仓库调拨入库单{dsData[i].DataMain.Rows[0]["LSH"].SqlDataBankToString()}失败,原因:{returnValue.response.content.message + insertresult}";
            //                }
            //            }
            //            //响应的原始报文,如果请求失败,从这里获取错误消息代码
            //            else
            //            {
            //                allResult += $"\r\n仓库调拨入库单，入参:{JsonConvert.SerializeObject(request)},\r\n返回:{JsonConvert.SerializeObject(response)}";
            //            }
            ...
            //            {
            //                //save rtsorderstatus to STOCKRETURNAPPROVE_XH_JDWMS table
            //                var insertresult = sqlHelper.ExecSql($"insert into CKDBDZK_XH_JDWMS(LSH,SYNCDATE,DBRKCKDCODE,TYPE) values('{dsData[i].DataMain.Rows[0]["LSH"].SqlDataBankToString()}',{createdate},'{returnValue.response.content.deliveryOrderCode}','{dsData[i].DataMain.Rows[0]["ORDERTYPE"].SqlDataBankToString()}')");
            //                if (string.IsNullOrEmpty(insertresult))
            //                {
            //                    allResult += $"\r\n仓库出库单{dsData[i].DataMain.Rows[0]["LSH"].SqlDataBankToString()}成功";
            //                }
            //                else
            //                {
            //                    sqlHelper.ExecSql($"insert into ERRORDATA_XH_JDWMS(GUID,ERRORNO,ERRORTYPE,MAINFIELD,ERRORDATE,ERRORMSG) values('{Guid.NewGuid():N}',{dsData[i].DataMain.Rows[0]["LSH"].SqlDataBankToString()},'dbckd','LSH',{createdate},'{returnValue.response.content.message}')");
            //                    allResult += $"\r\n仓库出库单{dsData[i].DataMain.Rows[0]["LSH"].SqlDataBankToString()}失败,原因:{returnValue.response.content.message + insertresult}";
            //                }
            //            }
            //            //响应的原始报文,如果请求失败,从这里获取错误消息代码
            //            else
            //            {
            //                allResult += $"\r\n仓库出库单，入参:{JsonConvert.SerializeObject(request)},\r\n返回:{JsonConvert.SerializeObject(response)}";
            //            }
            ...
            if (jobPara.jobCode == "test")
            {
                for (int i = 0; i < 100; i++)
                {
                    GetContext.WriteLog(LogType.Info, "test", $"测试一下{i}");
                }
                return 0;
            }
            return 0;
